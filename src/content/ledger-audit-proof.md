...
Halaman ini mendesain ledger, audit trail, dan proof. Ledger di sini bukan hanya buku uang. Ledger adalah catatan state change penting: siapa bertindak, atas instruksi apa, memakai sumber apa, apakah approval dibutuhkan, apakah approval diberikan, apa yang berubah, dan bukti apa yang membuktikan hasilnya.
...
Target halaman ini: lu bisa membuat evidence contract untuk setiap action class, lalu memeriksa pekerjaan agen dari record, bukan dari vibe.
...
## Anatomy of a Ledger Event

Gunakan field ini sebagai dasar.

| Field | Makna | Wajib untuk |
|---|---|---|
...
Jangan jadikan semua field wajib untuk semua hal. Tapi jangan pernah hilangkan actor, action, source, proof, state, dan time.
...
Event type memudahkan audit. Kalau semuanya masuk “task complete”, record akan jadi kabur.
...
- siapa review;
- siapa approve;
- apa yang dipakai sebagai sumber;
- apa yang berubah;
- bukti final apa;
...
| Final URL | kuat | membuktikan publikasi visible saat dicek |
...
- data berisi private material yang tidak boleh bocor;
...
- verified berarti proof final dicek;
...
|---|---|---|
| Agent said done | Bisakah manusia membuktikan tanpa percaya agen? | proof artifact ada |
...
Setiap completion claim butuh URL, diff, receipt, source list, or check output. Ini level minimum untuk working agents.
...
High-value record bisa di-hash atau signed. Approval punya TTL. Sensitive actions punya immutable-ish trail. Record bisa dibagikan secara terbatas tanpa private leakage.

## Done When

Halaman ini selesai untuk lu kalau setiap action penting punya:
...
Kalau agen tidak bisa menunjukkan proof, statusnya bukan done. Statusnya attempted.
