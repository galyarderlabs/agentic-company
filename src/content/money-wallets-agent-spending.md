...
Halaman ini membahas bagian paling berbahaya dari agentic company: uang. Bukan hanya wallet crypto. Bukan hanya transfer bank. Uang juga keluar lewat API calls, compute, paid SaaS, ads, contractor, domain, hosting, subscription upgrade, refund, chargeback, paid data, dan percobaan otomatis yang jalan terlalu lama.
...
Target halaman ini: lu bisa mendesain financial rights untuk setiap agen. Siapa boleh melihat data biaya. Siapa boleh membuat draft spend. Siapa boleh meminta payment. Siapa tidak boleh menyentuh wallet. Berapa batas per action, per hari, per minggu. Apa proof yang wajib ada. Apa yang terjadi kalau biaya aneh muncul.

Jika aturan uang tidak tertulis, agen belum siap memegang kerja serius.
...
| Tool spending | API token, model call, data provider, image/video generation | runaway cost, tagihan membesar diam-diam |
| Subscription change | upgrade SaaS, add seat, paid addon | biaya berulang tanpa review |
| Ads spend | campaign, boosted post, paid acquisition test | burn cepat dan public risk |
| Contractor spend | hire freelancer, approve invoice | legal/payment dispute |
| Refund/credit | customer refund, discount, account credit | revenue leakage |
| Wallet action | sign transaction, bridge, swap, gas fee | irreversible loss, chain/address error |
| x402 payment | pay-per-request service call | microspend bisa menumpuk |

Semua ini masuk financial action. Jangan batasi policy hanya pada “transfer uang”. Batasi semua action yang bisa membuat biaya.
...
| F5 | high-value financial action | hampir tidak cocok untuk agen otomatis | explicit owner approval every time |

Sebagian besar agen harus berhenti di F1 atau F2. F3 hanya untuk kerja yang berulang, nominal kecil, dan mudah dibuktikan. F4 perlu alasan teknis yang kuat. F5 tetap milik manusia.

## Spending Envelope

Spending envelope adalah batas tertulis untuk biaya yang boleh dipicu agen. Envelope harus spesifik, bukan “low cost”.
...
Jika envelope tidak punya expiry date, ia bisa menjadi permission permanen yang tidak pernah ditinjau.
...
- address tujuan dari mana datangnya;
...
Kalau menggunakan AgentKit atau alat serupa, tetap jangan terhipnotis oleh kemudahan integrasi. AgentKit membantu agen berinteraksi dengan wallet dan network action. Itu tidak menghapus kebutuhan limit, allowlist, approval, dan ledger.

## Wallet Action Classes

| Class | Contoh | Default rule |
|---|---|---|
| W0 observe | cek balance, cek transaction status | boleh read-only jika address bukan secret |
...
Jangan izinkan agen melakukan retry pembayaran tanpa batas. Retry adalah salah satu jalan paling cepat menuju tagihan tidak sadar.
...
Setiap request financial harus membawa lima data minimum:
...
Agent boleh salah membaca. Ledger event membuat kesalahan bisa ditemukan.
...
| Gas surprise | fee lebih tinggi dari nilai transaksi | max gas rule, abort threshold |
...
Setiap failure harus menghasilkan policy change. Kalau cuma “ingatkan agen”, failure akan balik.
...
### Level 4: Audited Financial Agent

Setiap biaya terhubung ke loop bisnis, output, approval, proof, dan review. Permissions punya expiry. Cost per agent bisa dilihat. Runaway loop punya hard stop.

## Done When

Halaman ini selesai untuk lu kalau setiap agen yang bisa menyentuh uang punya:

- financial level;
- allowed vendors atau network;
- per-action cap;
- daily dan weekly cap;
- approval threshold;
- auto-stop rule;
- ledger event requirement;
- proof requirement;
- alert route;
- expiry date;
- review cadence;
- rule untuk wallet, AgentKit, x402, API spend, subscription, dan refund.
