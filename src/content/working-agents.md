...
Halaman ini mengubah istilah Working Agents dari daftar nama keren menjadi desain kerja yang bisa dipakai. Setelah halaman sebelumnya membahas Paperclip atau ruang perusahaan untuk mencatat goal, task, approval, activity, dan cost, halaman ini menjawab pertanyaan berikut: agen apa saja yang layak dibuat, apa tugasnya, output apa yang harus mereka hasilkan, bagaimana mereka saling menyerahkan kerja, kapan manusia wajib masuk, dan kapan sebuah agen harus dipensiunkan.

Working agent bukan karakter tambahan. Working agent adalah peran kerja yang punya kontrak. Kontraknya menjelaskan pekerjaan berulang yang dia pegang, sumber yang boleh dibaca, alat yang boleh dipakai, output yang wajib ditinggalkan, proof yang harus ada, dan gate yang tidak boleh dilewati.
...
Target halaman ini sederhana: lu bisa membuka satu dokumen roster dan langsung tahu siapa mengerjakan apa, apa bukti kerjanya, siapa review, dan apa yang terjadi kalau dia gagal.

## Working Agent Bukan Persona

Persona menjawab pertanyaan: agen ini terdengar seperti siapa?

Working agent menjawab pertanyaan: pekerjaan apa yang selesai karena dia ada?

Keduanya tidak sama. Persona bisa membantu nada komunikasi, terutama untuk support, research, atau internal briefing. Tapi persona tidak boleh menggantikan kontrak kerja. Agen yang “visionary”, “friendly”, atau “ruthless” masih tidak berguna kalau outputnya tidak jelas.
...
Jangan mulai dari “gua butuh finance agent, sales agent, product agent”. Mulai dari loop bisnis yang berulang.
...
| Finance close | receipt intake, category check, anomaly flag | finance clerk agent, audit agent | ledger row, receipt link, exception note |
| Product maintenance | issue triage, repro, small patch proposal | product triage agent, coding agent, QA agent | issue ID, repro note, diff, test output |
...
Roster adalah daftar agen kerja yang hidup sekarang. Roster bukan wishlist. Jangan masukkan agen yang belum punya loop.

Roster minimum untuk tahap awal agentic company biasanya cukup berisi 3 sampai 7 agen aktif.
...
- **Checker**: agen yang memeriksa klaim, bukti, gate, dan risiko sebelum output dipakai.
- **Coordinator**: agen yang menjaga antrian, deadline, handoff, stale task, dan eskalasi lintas peran.
...
Roster ramping lebih aman. Agen sedikit dengan kontrak kuat mengalahkan dua puluh agen yang hanya membuat noise.
...
Working agents harus masuk ke loop kerja yang sama setiap kali. Tanpa loop, hasilnya akan berubah sesuai mood prompt.
...
Loop ini harus membosankan. Boring is the point. Agen yang bisa mengulang loop yang sama dengan proof lebih berharga daripada agen yang setiap hari menemukan cara baru untuk terlihat pintar.

## Handoff Packet

Agent-to-agent handoff harus pendek dan bisa diverifikasi. Jangan izinkan agen saling kirim monolog panjang tanpa status kerja.
...
Working agents gagal dengan pola yang bisa ditebak.
...
| Authority drift | agen membuat keputusan yang harusnya manusia | revoke action class, add gate |
...
Agen punya budget, access scope, state tracking, failure behavior, health checks, dan retirement criteria. Ia bisa bekerja di sekitar akun dan tool nyata tanpa kehilangan kendali manusia.
...
- outputnya duplikat dengan agen lain;
...
Jangan isi roster lebih dari tujuh agen di awal. Kalau daftar pertama lu panjang, itu biasanya tanda bisnis loop belum dipahami.

## Done When

Halaman ini selesai untuk lu kalau lu bisa membuat roster yang menjawab:

- agen mana yang aktif sekarang;
- loop bisnis apa yang mereka layani;
...
Kalau jawaban itu belum ada, jangan tambah agen. Perkuat kontrak kerja dulu.
